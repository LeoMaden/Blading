...
class Blade:
    ...
    def get_section(self, index: int) -> Section:
        ...
        if not 0 <= index < len(self.sections):
            raise IndexError(f"Section index {index} out of range [0, {len(self.sections)-1}]")
        return self.sections[index]
    
    def add_section(self, section: Section) -> None:
        """Add a new section to the blade."""
        # Check reference point consistency before adding
        if self.sections and section.reference_point != self.sections[0].reference_point:
            raise ValueError(f"Section reference point '{section.reference_point}' does not match blade reference point '{self.sections[0].reference_point}'")
    ...
    def set_all_reference_points(self, reference_point: Literal["leading_edge", "trailing_edge", "centroid"]) -> None:
    ...
    def _check_reference_point_consistency(self) -> bool:
        ...
        return all(section.reference_point == first_ref_point for section in self.sections)
    ...
    def get_thickness_at_position(self, normalized_position: float) -> NDArray:
        ...
        for section in self.sections:
            ...
            thickness = np.interp(normalized_position, section.normalised_arc_length, section.thickness)
    ...
    def plot_span_wise_properties(self, figsize=(15, 10)):
        ...
        labels = ['Leading Edge', '25% Chord', '50% Chord', '75% Chord', 'Trailing Edge']
        
        for pos, color, label in zip(positions, colors, labels):
            thickness = self.get_thickness_at_position(pos)
            axes[0, 1].plot(span_pos, thickness, 'o-', color=color, 
        ...
        fig.suptitle(f'Blade Span-wise Properties ({self.num_sections} sections)', fontsize=16)
    ...
    def plot_meridional_view(self, ax=None):
        ...
        if not sections_with_streamlines:
            ax.text(0.5, 0.5, 'No sections with stream lines available', 
        ...
        for i, section in enumerate(sections_with_streamlines):
            ...
            section.plot_meridional_streamline(
                ...
                show_feature_labels=(i == 0)  # Only show feature labels for first section
            )
